...
function Editar() {
  ...
  const { fun_context_entidad_update } = useEntidad();
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      idNombre: data?.nombreEntidad || "",
      idPosicion: data?.posicion || "",
      idTipoVela: data?.tipoVelaCodigo || "",
      idFechaRegistro: new Date(data?.date + " 00:00:00") || new Date(),
      idValor: data?.vAccion != null ? data.vAccion.toFixed(4) : "",
      idDescripcion: data?.description || "",
    },
  })
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        let o_response_1 = await fun_context_entidad_update(data.idEntidad, o_payload_1);

        if (o_response_1.validation === "OK") {
          if (o_response_1.response.result !== undefined) {
            fun_context_modal_message("S", "Mensaje de éxito", "Los datos se han actualizado correctamente.");
            o_navigate('/entidad');
          } else {
            fun_context_target_message("W", "Mensaje de advertencia", o_response_1.response.message);
          }
  ...
  let on_click_resetear = () => {

    Swal.fire({
      title: "Solicitud de confirmación",
      text: "¿Desea resetear los campos?",
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#089981",
      cancelButtonColor: "#F23645",
      ...
      <div className="flex justify-between w-full">
        ...
        <div className="flex gap-2">
          <Button variant="green" onClick={form.handleSubmit(onSubmit)}>
            <Icons icon="edit" />Actualizar
          </Button>
          <Button variant="blue" onClick={() => on_click_resetear()}>
            <Icons icon="refresh" />Resetear
          </Button>

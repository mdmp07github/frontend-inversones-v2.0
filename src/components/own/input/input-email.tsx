...
export default function InputEmail({ form, name, loading = false, disable, binding, label, placeholder, info }: VDMInputProps) {
            ...
            <FormField
               ...
               render={({ field }) => (
                  <FormItem>
                     ...
                     <FormControl>
                        <Input type="email" className={`${disable ? "pointer-events-none bg-neutral-100 dark:bg-input/10" : ""} ${label ? "" : "-mt-2"} ${hasError ? "border-destructive focus-visible:ring-destructive" : ""}`} placeholder={placeholder} {...field} />
                     </FormControl>
                     <FormMessage />
                  </FormItem>
               )}
